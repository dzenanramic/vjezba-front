...
function UploadingPage() {
  ...
  return (
    <div className="flex flex-col items-center min-h-[100dvh] pb-[calc(env(safe-area-inset-bottom)+72px)]">
      ...
      {/* Bottom action bar */}
      <div className="sticky bottom-0 left-0 right-0 mx-auto w-[95%] max-w-[100%] bg-background/80 backdrop-blur-sm flex gap-3 p-4 pt-2 pb-[calc(0.75rem+env(safe-area-inset-bottom))] border-t z-20">
        <input
          type="file"
          accept="image/*"
        ...
        >
          {loading ? "Processing image.." : "Remove background"}
        </button>
      </div>
    </div>
  );
